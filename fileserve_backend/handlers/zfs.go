...
// InstallZFS installs ZFS packages
func InstallZFS() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		switch pm {
		case "dnf", "yum":
			// Detect EL version for correct ZFS repo
			elVersion := detectELVersion()
			allOutput.WriteString(fmt.Sprintf("Detected EL version: %s\n", elVersion))

			// EL10 doesn't have an official zfs-release package yet
			if elVersion == "10" {
				allOutput.WriteString("EL10 detected - using --nogpgcheck for ZFS packages\n")
				commands = [][]string{
		...
		case "apt":
			// Ubuntu/Debian - ZFS is in universe repository
			// For Ubuntu 20.04+, zfsutils-linux is available
			// For older versions or Debian, may need to enable backports
			commands = [][]string{
				{"sudo", "apt", "update"},
				{"sudo", "apt", "install", "-y", "zfsutils-linux", "zfs-dkms"},
			}
		default:
			http.Error(w, "Unsupported package manager", http.StatusInternalServerError)
			return
		}
		...
		for _, cmdArgs := range commands {
			...
			if err != nil {
				...
				// Continue trying other commands for dependency installation
			}
		}

		// Try to load the module
		allOutput.WriteString("Attempting to load ZFS kernel module...\n")
		modOutput, modErr := exec.Command("sudo", "modprobe", "zfs").CombinedOutput()
		allOutput.WriteString(string(modOutput))
		if modErr != nil {
			allOutput.WriteString(fmt.Sprintf("Module load error: %s\n", modErr.Error()))
		}

		// Check if ZFS is now available
		_, zpoolErr := exec.LookPath("zpool")
		success := zpoolErr == nil
		...
		if success {
			response["message"] = "ZFS installation completed successfully"
		} else if lastError != nil {
			response["message"] = fmt.Sprintf("ZFS installation failed: %s", lastError.Error())
			response["error"] = lastError.Error()
		} else {
			response["message"] = "ZFS installation completed but zpool command not found"
		}
